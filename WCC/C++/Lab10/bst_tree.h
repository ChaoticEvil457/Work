...
*/
    ...
    public:
        ...
        virtual ~bst_tree() { clear(root_ptr); };
        ...
        long height() const { return (root_ptr == nullptr) ? -1 : bst_height(root_ptr); };
        ...
        template<typename P>
        void in_order(P f) { in_order(f, this->root_ptr); }

        template<typename P, typename E>
        void in_order(P f, E &extra) { in_order(f, this->root_ptr, extra); }
    ...
    protected:
        ...
        template<typename P>
        void in_order(P f, node *node_ptr);

        template<typename P, typename E>
        void in_order(P f, node *node_ptr, E &extra);
...
// Library facilities used: iomanip, iostream, stdlib
    ...
    template<typename T>
    template<typename P>
    void bst_tree<T>::in_order(P f, node *node_ptr) {
        if (node_ptr != nullptr) {
            in_order(f, node_ptr->left());
            f(node_ptr->data());
            in_order(f, node_ptr->right());
        }
    }

    template<typename T>
    template<typename P, typename E>
    void bst_tree<T>::in_order(P f, node *node_ptr, E &extra) {
        if (node_ptr != nullptr) {
            in_order(f, node_ptr->left(), extra);
            f(node_ptr->data(), extra);
            in_order(f, node_ptr->right(), extra);
        }
    }
